...
function MiddlePane({ puzzleData }) {
  ...
  function createBoard(difficulty, number) {
    ...
    if (difficulty == jsonSample.json1) {
      ...
      url =
        "https://sudokuapp-f0e20225784a.herokuapp.com/api/getpuzzle_devtest";
    } else {
      url = "https://sudokuapp-f0e20225784a.herokuapp.com/api/getpuzzle_easy";
    }
  ...
  function winEffects() {
    ...
    if (localStorage && localStorage.getItem("user_data")) {
      ...
      if (difficulty == jsonDifficulties.t1) {
        url =
          "https://sudokuapp-f0e20225784a.herokuapp.com/api/setusertime_easy";
        data = {
        ...
        url =
          "https://sudokuapp-f0e20225784a.herokuapp.com/api/setusertime_medium";
        data = {
          //username: localStorage.getItem("user_data").username,
          username: JSON.parse(localStorage.getItem("user_data")).username,
          puzzle_number: Number(puzzleData.number),
        ...
        url =
          "https://sudokuapp-f0e20225784a.herokuapp.com/api/setusertime_hard";
        data = {
          //username: localStorage.getItem("user_data").username,
          username: JSON.parse(localStorage.getItem("user_data")).username,
          puzzle_number: Number(puzzleData.number),
  ...
  function highlightAll(number) {
    for (var i = 0; i < 81; i++)
      if (allCells[i].textContent == selectedCell.textContent) {
        ...
        else thisCell.style.backgroundColor = "#B1FFCA";
      }
  }
  ...
  function unHighlightAll(number) {
    for (var i = 0; i < 81; i++) {
      if (allCells[i].textContent == number) {
        ...
        else thisCell.style.backgroundColor = "#FFFFFF";
      }
    }
  }
  ...
  const selectCell = (event) => {
    ...
    if (selectedCell.textContent != "") highlightAll(selectedCell.textContent);
  };
  ...
  function isValidSudoku() {
    // check rows
    for (let i = 0; i < 9; i++) if (!isValidSet(board[i])) return false;

    // check columns
    for (let i = 0; i < 9; i++) if (!isValidSet(getColumn(i))) return false;
  ...
  function getColumn(colIndex) {
    var column = [];
    for (let i = 0; i < 9; i++) column.push(board[i][colIndex]);
    return column;
  }
  ...
  return (
    <div className="Middle-pane">
      {puzzleCompleted === true && (
        <WinScreen time_taken={timeString} logged_in={logged_in} />
      )}
      <LandingTitle />
      {!timerHidden ? (
        <div className="clockContainer">
          <div className="clockText" id="clockText">
            0:00.0
          </div>
          <button className="clockButton" onClick={hideTimer}>
            Hide Timer
          </button>
        </div>
      ) : (
        <div className="clockContainer">
          <div className="clockText" id="clockText" style={{ display: "none" }}>
            0:00.0
          </div>
          <button className="clockButton" onClick={unhideTimer}>
            Show Timer
          </button>
        </div>
      )}
